  ...
  async function pump(runtime: AgentRuntime): Promise<void> {
    try {
      ...
      for await (const raw of runtime.query.stream) {
        ...
        for (const event of normalizeSdkMessage(msg, lastSessionId)) publish(runtime, event);
      }
      if (runtime.status !== "closed") {
        runtime.status = "closed";
        publish(runtime, { type: "error", message: "agent runtime closed", sessionId: lastSessionId });
      }
    } catch (err) {
      runtime.status = "failed";
      publish(runtime, { type: "error", message: err instanceof Error ? err.message : String(err), sessionId: runtime.sessionId });
    }
  }
